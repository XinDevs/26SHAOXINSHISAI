...
/**
 * @brief  非阻塞路口决策1：根据摄像头颜色 + 拓扑地图决定转向方向
...
/**
 * @brief  非阻塞路口决策2：根据摄像头颜色 + 拓扑地图决定转向方向区别于决策1 决策2在于目标就是回家，而不是前方的色块只是阻挡他回家的路，所以在决策逻辑上会更倾向于回家路线，除非颜色非常明确地指示了另一个方向，
 *         于是target_N这个参数在决策2中就没有意义了，因为不再以找到多少个标识为条件切换状态，而是直接在回家模式下执行决策2的逻辑。但是出家门口N1他是一个特殊的位置，如果没有色块指示他下一步该往哪走，那就默认出门右转，此时才算是真正开始了回家模式的决策2逻辑。
 * @retval TURN_LEFT(0) 或 TURN_RIGHT(1): 正常转向方向
 * @retval -1: 已到达终点(O点)，任务应结束
 * @note   此函数同时更新图论状态 (edge_visited, last_node, curr_node 等)
 */
