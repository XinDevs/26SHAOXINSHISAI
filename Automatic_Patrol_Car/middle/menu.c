 ...
 *  [11] Camera Test     (LEAF)
 *  [12] Gray Test       (LEAF)
 *  [13] Gray Analog Test(LEAF)
 *  [14] Flash Test      (LEAF)
 *  [15] OLED CN Test    (LEAF)
 *  [16] Buzzer LED Test (LEAF)
 *  [17] Settings        (NODE, 1 项)
 *  [18] PID Tune        (NODE, 4 项)
 *  [19] Left Wheel PID  (LEAF)
 *  [20] Right Wheel PID (LEAF)
 *  [21] Gray PID        (LEAF)
 *  [22] Yaw PID         (LEAF)
 */
...
static void actionLineTest(void)     { TargetYaw = CurrentYaw; Menu_StartTask(4U); } /* 任务4：颜色优先，无颜色随机转向 */
static void actionLineStopY(void)    { Menu_StartTask(5U); }   /* 任务5：循迹，走到Y路口停止 */
...
static void actionGrayAnalogTest(void) { Menu_SetMode(SYS_GRAY_ANALOG_TEST); }
...
static const MenuItem_t menuItems[] = {
    ...
    /* [4] 颜色优先，无颜色随机转向 */
    { "Color/Rand Y",    MENU_LEAF,  actionLineTest,     0, 0 },
    /* [5] 循迹，到Y路口转弯后停止 */
    ...
    /* [13] 灰度模拟量测试 */
    { "Gray Analog Test", MENU_LEAF, actionGrayAnalogTest, 0, 0 },
    /* [14] Flash 测试 */
    ...
    /* [17] 设置 */
    { "Settings",        MENU_NODE,  NULL,               18, 1 },
    /* [18] PID 调节 */
    { "PID Tune",        MENU_NODE,  NULL,               19, 4 },
    /* [19] 左轮 PID */
...
static const uint8_t s_topLevelItems[] = { 0U, 6U, 8U, 7U, 17U };
...
void Menu_ProcessKey(uint8_t key)
{
    ...
    /* 灰度测试：K4 返回 */
    if ((s_mode == SYS_GRAY_TEST) || (s_mode == SYS_GRAY_ANALOG_TEST)) {
        if (key == 4U) {
            s_mode = SYS_MENU;
        }
        return;
...
static void Menu_RenderGrayAnalogTest(void)
{
    CarTest_RenderGrayAnalog();
}
